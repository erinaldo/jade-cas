Public Class frmSI
    ...
    Private Sub LoadDR(ByVal DR_No As String)
        Dim query As String
        query = " SELECT  TransID, Date_DR AS Date, tblDR_Header.VCECode, VCEName  " & _
                " FROM    tblDR_Header INNER JOIN tblVCE_Master " & _
                " ON      tblDR_Header.VCECode = tblVCE_Master.VCECode " & _
                " WHERE   tblDR_Header.Status ='Open' AND TransID ='" & DR_No & "' "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            ...
            query = "  SELECT tblDR_Details.ItemCode, tblDR_Details.Description, tblDR_Details.UOM,  " & _
                    " 		tblDR_Details.QTY, Unit_Price, tblDR_Details.QTY * Unit_Price AS Amount " & _
                    "  FROM	tblSO INNER JOIN tblSO_Details " & _
                    "  ON		tblSO.TransID = tblSO_Details.TransID " & _
                    "  INNER JOIN tblDR_Header  " & _
                    "  ON		tblSO.TransID = tblDR_Header.SO_Ref " & _
                    "  INNER JOIN tblDR_Details " & _
                    "  ON		tblDR_Header.TransID = tblDR_Details.TransID " & _
                    "  WHERE  tblDR_Header.TransID = '" & DR_No & "' "
            SQL.ReadQuery(query)
            While SQL.SQLDR.Read
                dgvItemList.Rows.Add({SQL.SQLDR("ItemCode").ToString, SQL.SQLDR("Description").ToString, SQL.SQLDR("UOM").ToString,
                                      ...
                                      CDec(SQL.SQLDR("Amount")).ToString("N2"), "", "", CDec(SQL.SQLDR("Amount")).ToString("N2")})
            End While
            ComputeTotal()
        Else
            ClearText()
